...
// NewCallLocator connects to Redis and returns a CallLocator backed by it.
// If cfg.RedisAddr is empty, the locator is disabled: the recorder still
// runs, it just can't be discovered via Redis for operational APIs like the
// recording-split endpoint.
...
// locatorKey returns the Redis key used to store a call's owning pod
// address. The stored value is "ip:port" for the pod's HTTP API.
